...
export default {
  ...
  methods: {
    watsonPost() {
      axios.post('https://goodquotesapi.herokuapp.com/smart', { search: this.content })
      .then(res => axios.get(`https://goodquotesapi.herokuapp.com/tag/${res.data.keywords[randInt(res.data.keywords.length)].text}`))
      .then(smartQs => {
          this.smartRtn = true
